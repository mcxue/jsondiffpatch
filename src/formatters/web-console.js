...
let colors = {
  added: text => '[+]' + text,
  deleted: text => '[-]' + text,
  movedestination: text => '[→]' + text,
  moved: text => '[↑]' + text,
  unchanged: text => text,
  error: text => '[!]' + text,
  textDiffLine: text => '[t]' + text,
};

class WebConsoleFormatter extends BaseFormatter {
  constructor() {
    super();
...
export default WebConsoleFormatter;

let defaultInstance;

export const format = (delta, left) => {
  if (!defaultInstance) {
    defaultInstance = new WebConsoleFormatter();
  }
